...
class XiamiSpider(scrapy.Spider):  
	...
	for two in test:
		...
		c.append(two.split('\t')[1])# ID of an ixiami
	Dele2 = list(c)
	...
	def start_requests(self):
		urls = []
		for whichartist in self.c:
			re_url = str(whichartist)
			urls.append(re_url)
		for lib_url in urls:
	...
	def parse(self, response):
		artistnow = str(response).replace('<200', '').replace('>','').strip()
		if artistnow in self.c:
			# 排除掉混在普通音乐人里的虾米音乐人
			index3 = self.c.index(artistnow)
			# print(artistnow)
			each = response.xpath('//div[@class="glory-title-wrap"]/div/h1')
			ixiami_name = each.xpath('string(.)').extract()[0]#ixiami_name
			print(ixiami_name)
			for each2 in response.xpath('//div[@id="artist_info"]'):
				genre_address = each2.xpath('./table[1]/tbody[1]/tr[2]/td[2]/a[not(@class)]/@href').extract()#get every genre
				# print(genre_address)
				genre0 = []
				for many in genre_address:
					genre0.append(many.replace('http://www.xiami.com/genre/detail/sid/', ',').strip())#去掉多余字段，用，隔开多种风格
			...
			item['ixiami'] = artistnow
			item['ixiami_name'] = ixiami_name
			item['genre'] = genre
