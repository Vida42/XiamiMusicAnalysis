...
# 爬虾米音乐人的id（主页：http://i.xiami.com/gar）
...
def crawl():
    ...
    urls = 'http://www.xiami.com/song/'

    test=[] 
    with open('ixiami.txt','r', encoding="UTF-8") as f2:
        for each in f2:
            test.append(each.strip())
    a = []
    b = []
    for two in test:
    	a.append(two.split('\t')[0])# order of ID of a song
    	b.append(two.split('\t')[1])# ID of a song

    for song in b:
        urln = urls +  str(song)
        orders = str(a[b.index(song)])
        print(song)
        try:
            ...
            if rs.status_code == 200:
                ...
                demo = 'demo'
                if demo in location:
                    artist = location.split('/demo')[0]
                    # print(artist)
                    with open('302.txt', 'a', encoding="UTF-8") as f:
                        f.write(orders + '\t' + song + '\t' + artist + '\t' + '302_redirected' + '\t' + '302_redirected' + '\n')
                else:
                ...
                with open('404.txt', 'a', encoding="UTF-8") as f:
                    f.write(orders + '\t' + song + '\t' + '404_deleted' + '\t' + '404_deleted' + '\t' + '404_deleted' + '\n')
        ...
        with open ("ixiami.txt",'r+') as dele_r:
            Dele = dele_r.readlines()
        with open ("ixiami.txt", 'w+') as dele_w:
