...
class XiamiSpider(scrapy.Spider):  
    ...
    def parse(self, response):
        ...
        if response.xpath('//span[@class="counts"]') !=[]:
            allpage = response.xpath('//span[@class="counts"]/text()').extract()[0]
            counts = allpage.replace('首', '').strip()
            if int(counts) <= 100:
                ...
                if response.xpath('//a[@class="p_redirect_l"]') !=[]:#is not None:
                    ...
                    with open ("Xu2_4.txt", 'w') as dele_w:
                        for i in self.Dele:
                            dele_w.write(str(i)+'\n')
                    print(userid)
        else:
